...
namespace WebAPI_2.Controllers
{
     ...
     public class WebApiController : ControllerBase
     {
          ...
          public async Task<IActionResult> PostFileData([FromBody] string jsonData)
          {
               ...
               {
                    Category = Call_TC_API(ecall Number);//https://data.tc.gc.ca/v1.3/api/eng/vehicle-recall-database/recall-summary/recall-number/2016253
                    Add_To_Input_Data(Category);
               }
          ...
          public async Task<IActionResult> Search(string category)
          {
                ...
                *   foreach(API_Object_Data in jsonFileData)
	               {
		               if ((API_Object_Data.CATEGORY_ETXT == category) || (API_Object_Data.CATEGORY_FTXT == category))
		               {
