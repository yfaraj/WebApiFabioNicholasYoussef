...
namespace WebAPI_4.Controllers
{
     ...
     public class WebApiController : ControllerBase
     {
          ...
          public async Task<IActionResult> PostFileData([FromBody] string jsonData)
          {
               ...
               {
                    NotificationType = Call_TC_API(ecall Number);//https://data.tc.gc.ca/v1.3/api/eng/vehicle-recall-database/recall-summary/recall-number/2016253
                    Add_To_Input_Data(NotificationType);
               }
          ...
          public async Task<IActionResult> Search(string notificationType)
          {
                ...
                *   foreach(API_Object_Data in jsonFileData)
	               {
		               if ((API_Object_Data.NOTIFICATION_TYPE_ETXT == notificationType) || (API_Object_Data.NOTIFICATION_TYPE_FTXT == notificationType))
		               {
